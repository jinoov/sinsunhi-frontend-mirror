...
let make = () => {
            ...
            <div className=%twc("flex-1 px-3 lg:px-0")>
              <div className=%twc("flex mt-2 flex-col sm:flex-row")>
                ...
                <div
                  ...
                  <label
                    htmlFor="product-name" className=%twc("whitespace-nowrap mr-2 mb-1 sm:mb-0")>
                    {j`상품명`->React.string}
                  </label>
  ...
  <FeatureFlagWrapper featureFlag=#HOME_UI_UX fallback=oldUI>
    <div className=%twc("sm:mt-4 bg-white")>
      <Formula.Text.Headline
        weight=#bold size=#lg className=%twc("mb-7 mt-10 ml-[50px] hidden lg:block")>
        {"단품 확인"->React.string}
      </Formula.Text.Headline>
      <form onSubmit={handleOnSubmit} className=%twc("py-7 px-4 lg:py-0 lg:px-[50px]")>
        <div className=%twc("py-3 flex flex-col text-sm bg-[#FAFBFC] rounded-xl")>
          <div className=%twc("flex flex-col lg:flex-row")>
            <div className=%twc("w-32 font-bold pl-3 whitespace-nowrap lg:pl-7 lg:pt-3")>
              {j`검색`->React.string}
            </div>
