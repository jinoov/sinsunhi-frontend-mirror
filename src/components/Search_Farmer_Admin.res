module FormFields = Query_Farmer_Form_Admin.FormFields
module Form = Query_Farmer_Form_Admin.Form
...
let make = () => {
  ...
  let onSubmit = ({state}: Form.onSubmitAPI) => {
    ...
    let producerCode = state.values->FormFields.get(FormFields.ProducerCode)

    let {makeWithDict, toString} = module(Webapi.Url.URLSearchParams)

    router.query->Js.Dict.set("name", name)
    router.query->Js.Dict.set("producer-code", producerCode)
  ...
  let form: Form.api = Form.use(
    ...
    ~onSubmit,
    ~initialState=Query_Farmer_Form_Admin.initialState,
    ~schema={
  ...
  React.useEffect1(_ => {
    ...
    ->Garter.Array.forEach(entry => {
      ...
      if k === "name" {
        ...
        FormFields.ProducerCode->form.setFieldValue(v, ~shouldValidate=false, ())
      }
    })
  ...
  let handleOnReset = (
    _ => {
      ...
      FormFields.ProducerCode->form.setFieldValue("", ~shouldValidate=false, ())
    }
  )->ReactEvents.interceptingHandler
            ...
            <div className=%twc("flex")>
              <div
                ...
                <Input
                  ...
                  value={form.values->FormFields.get(FormFields.ProducerCode)}
                  onChange={FormFields.ProducerCode->form.handleChange->ReForm.Helpers.handleChange}
                  error={FormFields.ProducerCode->Form.ReSchema.Field->form.getFieldError}
                  tabIndex=1
                />
              </div>
