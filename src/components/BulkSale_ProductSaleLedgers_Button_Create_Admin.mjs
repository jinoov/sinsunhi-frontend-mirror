...
import * as BulkSaleProductSaleLedgersButtonCreateAdminMutation_graphql from "../__generated__/BulkSaleProductSaleLedgersButtonCreateAdminMutation_graphql.mjs";

function commitMutation(environment, variables, optimisticUpdater, optimisticResponse, updater, onCompleted, onError, uploadables, param) {
  return RelayRuntime.commitMutation(environment, {
              mutation: BulkSaleProductSaleLedgersButtonCreateAdminMutation_graphql.node,
              variables: BulkSaleProductSaleLedgersButtonCreateAdminMutation_graphql.Internal.convertVariables(variables),
              onCompleted: (function (res, err) {
                  if (onCompleted !== undefined) {
                    return Curry._2(onCompleted, BulkSaleProductSaleLedgersButtonCreateAdminMutation_graphql.Internal.convertResponse(res), (err == null) ? undefined : Caml_option.some(err));
                  }
...
var MutationCreate_productPackageMassUnit_decode = BulkSaleProductSaleLedgersButtonCreateAdminMutation_graphql.Utils.productPackageMassUnit_decode;
...
var LedgerFile = {
  make: BulkSale_ProductSaleLedgers_Button_Create_Admin$LedgerFile
};

function makeInputCreate(bulkSaleApplicationId, path, date, wholesalerId, ledgers) {
  return {
...
function makeEntryInputCreate(grade, price, amount, unit, volume) {
  return {
          grade: grade,
          price: price,
          quantity: {
  ...
  return React$1.createElement(React$1.Fragment, undefined, React$1.createElement("article", {
                  className: "mb-5"
                      ...
                      className: "flex gap-2 mt-2"
                        ...
                        }, "업로드")), Belt_Option.mapWithDefault(Garter_Array.first(Belt_Array.keepMap(formErrors, (function (error) {
                                var match = error[1];
                                if (typeof match === "object" && match.NAME === "ErrorPath") {
                                  return match.VAL;
                                }
                  ...
                  className: "flex gap-2 mt-4"
                      ...
                      className: "w-1/3"
                          ...
                          error: Garter_Array.first(Belt_Array.keepMap(formErrors, (function (error) {
                                      var match = error[1];
                                      if (typeof match === "object" && match.NAME === "ErrorDate") {
                                        return match.VAL;
                                      }
                          ...
                          className: "flex mr-2"
                              ...
                              onClick: (function (param) {
                                  var inputNewEntries = Belt_Array.keepMap(Belt_MapString.valuesToArray(newEntries), (function (entry) {
                                          var input = V.ap(V.ap(V.ap(V.ap(V.map(makeEntryInputCreate, V.nonEmpty({
                                                                    NAME: "ErrorGrade",
                                                                    VAL: "등급을 입력해주세요"
                                                                  }, entry.grade)), V.$$int({
                                                                NAME: "ErrorPrice",
                                  ...
                                  var input = V.ap(V.ap(V.ap(V.ap(V.map(makeInputCreate, V.nonEmpty([
                                                            "not-existing-ledger-id",
                                                            {
                                                              NAME: "ErrorApplicationId",
                                                              VAL: "applicationId가 필요합니다"
                                                            }
                                                          ], applicationId)), V.$$Option.nonEmpty([
                                                        "not-existing-ledger-id",
                                                        {
                                                          NAME: "ErrorPath",
                                                          VAL: "파일을 업로드해주세요"
                                                        }
                                                      ], path)), V.$$Option.nonEmpty([
                                                    "not-existing-ledger-id",
                                                    {
                                                      NAME: "ErrorDate",
                                                      VAL: "날짜를 입력해주세요"
                                                    }
                                                  ], IsValid(new Date(date)) ? date : undefined)), V.$$Option.nonEmpty([
                                                "not-existing-ledger-id",
                                                {
                                                  NAME: "ErrorWholesalerId",
                                                  VAL: "출하시장, 법인명을 선택해주세요"
                                                }
                                              ], wholesalerId)), V.pure(inputNewEntries));
                                  if (input.TAG === /* Ok */0) {
                                    Curry.app(mutate, [
                                          (function (err) {
                                              console.log(err);
...
var make = BulkSale_ProductSaleLedgers_Button_Create_Admin;
