...
let make = (~userType: CustomHooks.Auth.role, ~requestUrl, ~buttonText=?, ~bodyOption=?) => {
  ...
  <>
    <button
      className=%twc(
        "text-white font-bold p-3 w-28 bg-green-gl rounded-xl focus:outline-none hover:bg-green-gl-dark whitespace-nowrap"
      )
      onClick={_ => setIsShowRequest(._ => Dialog.Show)}>
      {buttonText->Option.getWithDefault(j`다운로드`)->React.string}
    </button>
    <Dialog
