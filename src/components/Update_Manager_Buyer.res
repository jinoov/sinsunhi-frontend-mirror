...
module Mutation = %relay(`
  mutation UpdateManagerBuyer_Mutation($input: UpdateUserInput!) {
...
let make = (~isOpen, ~onClose, ~defaultValue="") => {
  let (manager, setManager, _) = ValidatedState.use(
  ...
  let handleOnChange = e => {
    ...
    setManager(value, ~shouldValidate=true, ())
  }
  ...
  let handleOnSubmit = (
    _ => {
      mutate(
        ~variables={
          input: Mutation.make_updateUserInput(~manager, ()),
        },
            ...
            <div className=%twc("flex flex-col ")>
              <div className=%twc("flex flex-col mb-10")>
                <div className=%twc("mb-2")>
                  <label className=%twc("font-bold")> {`담당자명`->React.string} </label>
                </div>
                <input
                  value={manager}
              ...
              <button
                className={cx([
                  ...
                  manager == "" ? %twc("bg-disabled-L2") : %twc("bg-green-500"),
                ])}
