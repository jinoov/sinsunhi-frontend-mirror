...
module NewUI = {
  ...
  let make = () => {
    <div
      className=%twc(
        "container lg:max-w-[1280px] lg:min-w-[872px] flex flex-col rounded-sm bg-white shadow-[0px_10px_40px_10px_rgba(0,0,0,0.03)]  w-full mt-4 "
      )>
      <RadixUI.Accordian.RootMultiple _type=#multiple>
        <RadixUI.Accordian.Item value="guide-1">
          <RadixUI.Accordian.Header>
            <RadixUI.Accordian.Trigger
              className=%twc("w-full focus:outline-none accordian-trigger")>
              <div
                className=%twc("flex justify-between py-6 px-7 bg-white border-b border-gray-100")>
                <span className=%twc("font-bold")>
                  {j`주문서 업로드 사용설명서`->React.string}
                </span>
                <IconArrowSelect
