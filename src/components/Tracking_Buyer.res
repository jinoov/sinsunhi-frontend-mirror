        ...
        Helper.Option.map2(order.courierCode, order.invoice, (courierCode, invoice) => {
          <>
            ...
            <button
              type_="button"
              className=%twc(
                "px-3 max-h-10 bg-green-gl-light text-green-gl font-bold rounded-lg whitespace-nowrap py-1 mt-2"
              )
          ...
          <button
            type_="button"
            className=%twc(
              "px-3 max-h-10 bg-gray-100 text-gray-300 rounded-lg whitespace-nowrap py-1 mt-2"
            )
            onClick={openPopupPostFormData}
            disabled=true>
            {j`조회하기`->React.string}
