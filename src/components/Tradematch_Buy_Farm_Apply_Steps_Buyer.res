module Query = {
  module TradematchDeliveryPolicy = %relay(`
       query TradematchBuyFarmApplyStepsBuyer_TradematchDeliveryPolicy_Query {
...
module Mutation = {
  module CreateTradematchDemand = %relay(`
    mutation TradematchBuyFarmApplyStepsBuyer_CreateTradematchDemand_Mutation(
      $connections: [ID!]!
  ...
  module PartialUpdateTradematchDemand = %relay(`
    mutation TradematchBuyFarmApplyStepsBuyer_PartialUpdateTradematchDemand_Mutation(
      $id: ID!
...
module Grade = {
  ...
  let make = (
    ...
    >,
    ~product: TradematchBuyFarmProductApplyBuyer_MatchingProduct_Query_graphql.Types.response_product_MatchingProduct,
    ~connectionId,
  ) => {
    let router = Next.Router.useRouter()
    let {router: {toNext}} = CustomHooks.FarmTradematchStep.use()
...
module Count = {
  ...
  let make = (
    ...
    let minimumCount = 1
    let maximumCount = 999

    let unitText = `박스`

    let increaseCount = () => {
      setCount(.prev => {
        switch prev >= maximumCount {
        | true => prev
        | false => prev + 1
        }
      })
    ...
    let (updateMutate, isUpdateMutating) = Mutation.PartialUpdateTradematchDemand.use()
    let isValidCount = count >= minimumCount
    ...
    <div>
      ...
      <div className=%twc("mx-5")>
        <div
          ...
          <div className=%twc("flex justify-between items-center")>
            ...
            <div className=%twc("mx-3 text-center flex items-center")>
              <input
                type_="text"
                value={count->Int.toFloat->Locale.Float.show(~digits=0)}
                onChange={e => {
                  let value = (e->ReactEvent.Synthetic.target)["value"]
                  let nextCount = value->convertNumberInputValue->Int.fromString
                  switch nextCount {
                  | Some(nextCount') =>
                    switch nextCount' >= maximumCount {
                    | true => setCount(._ => maximumCount)
                    | false => setCount(._ => nextCount')
                    }
                  | None => setCount(._ => 0)
                  }
                }}
              ...
              />
              <div className=%twc("ml-2")> {unitText->React.string} </div>
            </div>
            <button
      ...
      <Common.FloatingButton
        handleClickButton={handleClickButton} disabled={!isValidCount || isUpdateMutating}
      />
    </div>
  }
}
