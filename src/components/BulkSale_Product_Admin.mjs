...
function BulkSale_Product_Admin$Item$Table(props) {
  var product = use(props.node.fragmentRefs);
  return React$1.createElement(React$1.Fragment, undefined, React$1.createElement("li", {
                  className: "grid grid-cols-7-admin-bulk-sale-product"
                }, React$1.createElement("div", {
                      className: "h-full flex flex-col justify-center px-4 py-2"
                    }, Format(new Date(product.createdAt), "yyyy/MM/dd HH:mm")), React$1.createElement("div", {
                      className: "h-full flex flex-col justify-center px-4 py-2"
                    }, React$1.createElement(Select_BulkSale_Campaign_Status.make, {
                          product: product,
                          refetchSummary: props.refetchSummary
                        })), React$1.createElement("div", {
                      className: "h-full flex flex-col justify-center px-4 py-2"
                    }, "" + product.productCategory.crop.name + " > " + product.productCategory.name + ""), React$1.createElement("div", {
                      className: "h-full flex flex-col justify-center px-4 py-2"
                    }, "" + Locale.Float.show(undefined, product.estimatedPurchasePriceMin, 0) + "원~" + Locale.Float.show(undefined, product.estimatedPurchasePriceMax, 0) + "원"), React$1.createElement("div", {
                      className: "h-full flex flex-col justify-center px-4 py-2"
                    }, "" + product.preferredGrade + " " + product.preferredQuantity.display + ""), React$1.createElement("div", {
                      className: "h-full flex flex-col justify-center px-4 py-2"
                    }, "" + String(product.estimatedSellerEarningRate) + "%"), React$1.createElement("div", {
                      className: "h-full flex justify-center items-center px-4 py-1"
                    }, React.createElementWithKey(BulkSale_Product_Update_Button.make, {
                          product: product
                        }, product.id))));
}
...
function BulkSale_Product_Admin(props) {
  return React$1.createElement(BulkSale_Product_Admin$Item$Table, {
              node: props.node,
              refetchSummary: props.refetchSummary
            });
}
