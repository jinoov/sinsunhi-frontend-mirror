...
module Style = {
  let container = %twc("w-full h-9 pl-9 py-2 pr-3")
  let box = %twc(
    "rounded-lg border border-[#DCDFE3] focus:outline-none focus:ring-1-gl focus:border-border-active focus:ring-opacity-100 remove-spin-button"
  )
  let text = %twc("text-sm text-[#1F2024] placeholder:text-[#C3C5C9]")
}
...
module BuyerList = {
  module Query = %relay(`
    query SearchAdminAdminQuery($name: String!) {
      users(name: $name, roles: [ADMIN]) {
        edges {
          node {
            id
            name
...
module SelectedItem = {
  ...
  let make = (~label, ~select) => {
    ...
    <div className=%twc("relative")>
      <input
        type_="text"
        className={cx([Style.container, Style.box, Style.text])}
...
@react.component
let make = (~placeholder=`유저를 검색해주세요.`, ~value, ~onChange) => {
