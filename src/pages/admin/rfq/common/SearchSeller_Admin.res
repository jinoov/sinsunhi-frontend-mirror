...
module Scroll = {
  ...
  let make = (~children) => {
    ...
    <Root className=%twc("max-h-[120px] flex flex-col")>
...
module ResultItem = {
  @react.component
  let make = (~id, ~label, ~select) => {
    <div
      className=%twc("w-full max-w-[185px] px-3 py-2 rounded-lg hover:bg-gray-100 truncate")
...
module SellerList = {
  module Query = %relay(`
    query SearchSellerAdminQuery($name: String!) {
      users(name: $name, roles: [PRODUCER]) {
        edges {
          node {
            id
            name
      ...
      <ResultBox>
        <Scroll>
          ...
          ->Array.map(({node: {id, name}}) => {
            <ResultItem key=id id label=name select />
          })
          ->React.array}
        </Scroll>
      </ResultBox>
...
module SearchResult = {
  ...
  let make = (~keyword, ~select) => {
    switch keyword {
    | "" => <Empty />
    | keyword' => <SellerList keyword=keyword' select />
    }
  }
}
