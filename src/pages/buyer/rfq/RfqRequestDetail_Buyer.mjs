...
function RfqRequestDetail_Buyer$StatusLabel(Props) {
  ...
  if (status === "WAITING_FOR_QUOTATION" || status === "DRAFT" || status === "READY_TO_REQUEST") {
    return null;
...
function RfqRequestDetail_Buyer$BottomButton(Props) {
  ...
  var router = Router.useRouter();
  if (!(status === "WAITING_FOR_QUOTATION" || status === "REQUEST_CANCELED" || status === "REQUEST_PROCESSED" || status === "READY_TO_REQUEST") && status === "DRAFT") {
...
function RfqRequestDetail_Buyer$Title(Props) {
  ...
  return React.createElement(DS_Title.Normal1.Root.make, {
              children: status === "WAITING_FOR_QUOTATION" || status === "READY_TO_REQUEST" ? React.createElement(DS_Title.Normal1.TextGroup.make, {
                      title1: "요청 중인 견적서입니다.",
...
function RfqRequestDetail_Buyer$Detail(Props) {
  ...
  var node = match.node;
  React.useEffect((function () {
          var arrItem = Belt_Array.map(Belt_Option.mapWithDefault(node, [], (function (node$p) {
                      return node$p.items.edges;
                    })), (function (edge) {
                  return edge.node;
                }));
          DataGtm.push({
                event: "Expose_view_RFQ_Livestock_Status_QuotationList",
                totalCount: arrItem.length,
                waitingForOrder: Belt_Array.keep(arrItem, (function (x) {
                        return x.status === "WAITING_FOR_ORDER";
                      })).length,
                machedFailed: Belt_Array.keep(arrItem, (function (x) {
                        return x.status === "MATCH_FAILED";
                      })).length,
                ordered: Belt_Array.keep(arrItem, (function (x) {
