    ...
    <div className=%twc("mt-7 mb-10")>
      ...
      <div className=%twc("py-[14px] mx-5 bg-gray-100 rounded-xl ")>
        <ul>
          ...
          item.brands.edges->Garter.Array.isEmpty {
          | true => <Listitem.Quotation label={`브랜드`} text={`브랜드 무관`} />
          | false =>
            <Listitem.Quotation
              label={`브랜드`}
              text={item.brands.edges->Array.map(edge => {
                edge.node.name
              }) |> Js.Array.joinWith(", ")}
            />
          }}
...
module ConfirmButton = {
  ...
  let make = (
    ...
    let createOrder = _ => {
      ...
      let trackDataWhenOrder = () =>
        {
          "event": "click_yes_rfq_livestock_order_check_popup",
          "request_id": item.request.id,
          "request_item_id": item.id,
          "selected_quotation_id": quotation.node.id,
        }
    ...
    <>
      <DS_Dialog.Popup.Root>
        <DS_Dialog.Popup.Trigger asChild=true>
          <DS_ButtonContainer.Floating1 label={`주문하기`} />
...
module ConfirmContent = {
  module Drawer = {
    ...
    let make = (~item: RfqItemDetailBuyer_Current_Item_Query_graphql.Types.response_node) => {
      ...
      let togglePopup = _ => setDrawerShow(._ => !isDrawerShow)
    ...
    let make = (~item: RfqItemDetailBuyer_Current_Item_Query_graphql.Types.response_node) => {
      <div
        ...
        <ul>
          ...
          {switch // Brand 선택 개선 이후 - 새롭게 추가된 필드의 데이터를 보여줌.
          item.brands.edges->Garter.Array.isEmpty {
          | true => <Listitem.Normal label={`브랜드`} text={`브랜드 무관`} />
          | false =>
            <Listitem.Normal
              label={`브랜드`}
              text={item.brands.edges->Array.map(edge => {
...
module Detail = {
  ...
  let make = (~itemId) => {
    ...
    React.useEffect0(_ => {
      ...
      | Some(item) =>
        {
          "event": "view_order_rfq_livestock_quotation_detail",
          "request_id": item.request.id,
          "request_item_id": item.id,
          "selected_quotation_ids": item.selectedQuotations.edges->Array.map(x => x.node.id),
        }
        ->DataGtm.mergeUserIdUnsafe
        ->DataGtm.push
      | None => ()
