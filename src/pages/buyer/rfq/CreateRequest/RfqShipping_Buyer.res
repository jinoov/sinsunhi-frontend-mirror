module Query = {
  module CurrentRequest = RfqBasket_Buyer.Query.CurrentRequest
...
module Shipping = {
  ...
  let make = (~requestId) => {
    ...
    let queryData = Query.CurrentRequest.use(~variables={requestIds: Some([requestId])}, ())
    ...
    let (updateRfqRequest, isMutatingRfqRequest) = Mutation.UpdateDeliveryAddress.use()
    let (updateTermAgreement, isMutatingTermAgreement) = Mutation.UpdateTermAgreement.use()
    ...
    <div
      ...
      <DS_ButtonContainer.Floating1
        ...
        disabled={!isValidItems || isMutatingRfqRequest || isMutatingTermAgreement}
      />
      <AddressDrawer
