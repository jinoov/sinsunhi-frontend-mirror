module Result = {
  module Query = %relay(`
        query PCInterestedProductAddSearchBuyerQuery(
          $cursor: String
          $count: Int!
          $name: String
        ) {
          ...PCInterestedProductAddSearchBuyerFragment
            @arguments(count: $count, cursor: $cursor, name: $name)
        }
    `)

  module Fragment = %relay(`
    fragment PCInterestedProductAddSearchBuyerFragment on Query
    @refetchable(queryName: "PCInterestedProductAddSearchQuery")
    @argumentDefinitions(
      ...
      searchProducts(
        ...
        types: [MATCHING]
        name: $name
      ) @connection(key: "PCInterestedProductAddSearch_searchProducts") {
        edges {
          cursor
          node {
            id
            ...PCInterestedProductAddItemBuyer_Fragment
          }
        }
      }
    }
  ...
  let make = (~query, ~checkedSet, ~onItemClick) => {
...

    {
      ...
      | edges =>
        <>
          <ul className=%twc("w-full grid grid-cols-4 gap-4 mt-5 overflow-y-scroll flex-1")>
            {edges
            ->Array.map(({cursor, node}) => {
              <PC_InterestedProduct_Add_Item_Buyer
                key={cursor}
...
let make = (~checkedSet, ~onItemClick) => {
  ...
  <RescriptReactErrorBoundary fallback={_ => <div />}>
    <PC_InterestedProduct_SearchInput_Buyer
