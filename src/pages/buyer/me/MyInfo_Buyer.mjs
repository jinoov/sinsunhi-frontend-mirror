// Generated by ReScript, PLEASE EDIT WITH CARE

import * as Curry from "rescript/lib/es6/curry.js";
import * as React from "@rescript/react/src/React.mjs";
import * as React$1 from "react";
import * as RelayEnv from "../../../constants/RelayEnv.mjs";
import * as IconArrow from "../../../components/svgs/IconArrow.mjs";
import Link from "next/link";
import * as Js_promise from "rescript/lib/es6/js_promise.js";
import * as Caml_option from "rescript/lib/es6/caml_option.js";
import * as Router from "next/router";
import * as ReactRelay from "react-relay";
import * as DeviceDetect from "../../../bindings/DeviceDetect.mjs";
import * as Footer_Buyer from "../../../components/Footer_Buyer.mjs";
import * as Header_Buyer from "../../../components/Header_Buyer.mjs";
import * as Authorization from "../../../utils/Authorization.mjs";
import * as RescriptRelay from "rescript-relay/src/RescriptRelay.mjs";
import * as RelayRuntime from "relay-runtime";
import Format from "date-fns/format";
import SubMonths from "date-fns/subMonths";
import * as GnbBannerList_Buyer from "../../../components/GnbBannerList_Buyer.mjs";
import * as MyInfo_Layout_Buyer from "./components/MyInfo_Layout_Buyer.mjs";
import * as MyInfo_Skeleton_Buyer from "./components/MyInfo_Skeleton_Buyer.mjs";
import * as RescriptRelay_Internal from "rescript-relay/src/RescriptRelay_Internal.mjs";
import * as MyInfo_Cash_Remain_Buyer from "./components/MyInfo_Cash_Remain_Buyer.mjs";
import * as ShopCategorySelect_Buyer from "../../../components/ShopCategorySelect_Buyer.mjs";
import * as MyInfoBuyer_Query_graphql from "../../../__generated__/MyInfoBuyer_Query_graphql.mjs";
import * as RescriptReactErrorBoundary from "@rescript/react/src/RescriptReactErrorBoundary.mjs";
import * as MyInfo_ProfileSummary_Buyer from "./components/MyInfo_ProfileSummary_Buyer.mjs";
import * as MyInfo_Processing_Order_Buyer from "./components/MyInfo_Processing_Order_Buyer.mjs";
import * as MyInfo_Profile_Complete_Buyer from "./components/MyInfo_Profile_Complete_Buyer.mjs";

function use(variables, fetchPolicy, fetchKey, networkCacheConfig, param) {
  var data = ReactRelay.useLazyLoadQuery(MyInfoBuyer_Query_graphql.node, RescriptRelay_Internal.internal_cleanObjectFromUndefinedRaw(MyInfoBuyer_Query_graphql.Internal.convertVariables(variables)), {
        fetchKey: fetchKey,
        fetchPolicy: RescriptRelay.mapFetchPolicy(fetchPolicy),
        networkCacheConfig: networkCacheConfig
      });
  return RescriptRelay_Internal.internal_useConvertedValue(MyInfoBuyer_Query_graphql.Internal.convertResponse, data);
}

function useLoader(param) {
  var match = ReactRelay.useQueryLoader(MyInfoBuyer_Query_graphql.node);
  var loadQueryFn = match[1];
  var loadQuery = React$1.useMemo((function () {
          return function (param, param$1, param$2, param$3) {
            return Curry._2(loadQueryFn, MyInfoBuyer_Query_graphql.Internal.convertVariables(param), {
                        fetchPolicy: param$1,
                        networkCacheConfig: param$2
                      });
          };
        }), [loadQueryFn]);
  return [
          Caml_option.nullable_to_opt(match[0]),
          loadQuery,
          match[2]
        ];
}

function $$fetch(environment, variables, onResult, networkCacheConfig, fetchPolicy, param) {
  ReactRelay.fetchQuery(environment, MyInfoBuyer_Query_graphql.node, MyInfoBuyer_Query_graphql.Internal.convertVariables(variables), {
          networkCacheConfig: networkCacheConfig,
          fetchPolicy: RescriptRelay.mapFetchQueryFetchPolicy(fetchPolicy)
        }).subscribe({
        next: (function (res) {
            Curry._1(onResult, {
                  TAG: /* Ok */0,
                  _0: MyInfoBuyer_Query_graphql.Internal.convertResponse(res)
                });
          }),
        error: (function (err) {
            Curry._1(onResult, {
                  TAG: /* Error */1,
                  _0: err
                });
          })
      });
}

function fetchPromised(environment, variables, networkCacheConfig, fetchPolicy, param) {
  var __x = ReactRelay.fetchQuery(environment, MyInfoBuyer_Query_graphql.node, MyInfoBuyer_Query_graphql.Internal.convertVariables(variables), {
          networkCacheConfig: networkCacheConfig,
          fetchPolicy: RescriptRelay.mapFetchQueryFetchPolicy(fetchPolicy)
        }).toPromise();
  return Js_promise.then_((function (res) {
                return Promise.resolve(MyInfoBuyer_Query_graphql.Internal.convertResponse(res));
              }), __x);
}

function usePreloaded(queryRef, param) {
  var data = ReactRelay.usePreloadedQuery(MyInfoBuyer_Query_graphql.node, queryRef);
  return RescriptRelay_Internal.internal_useConvertedValue(MyInfoBuyer_Query_graphql.Internal.convertResponse, data);
}

function retain(environment, variables) {
  var operationDescriptor = RelayRuntime.createOperationDescriptor(MyInfoBuyer_Query_graphql.node, MyInfoBuyer_Query_graphql.Internal.convertVariables(variables));
  return environment.retain(operationDescriptor);
}

var Query = {
  Operation: undefined,
  Types: undefined,
  use: use,
  useLoader: useLoader,
  $$fetch: $$fetch,
  fetchPromised: fetchPromised,
  usePreloaded: usePreloaded,
  retain: retain
};

function MyInfo_Buyer$PC(props) {
  var to = new Date();
  var from = SubMonths(to, 1);
  var queryData = use({
        from: Format(from, "yyyyMMdd"),
        to: Format(to, "yyyyMMdd")
      }, undefined, undefined, undefined, undefined);
  var match = queryData.viewer;
  if (match === undefined) {
    return null;
  }
  var query = match.fragmentRefs;
  return React$1.createElement(MyInfo_Layout_Buyer.make, {
              query: query,
              children: React$1.createElement("div", {
                    className: "ml-4 flex flex-col w-full items-stretch"
                  }, React$1.createElement("div", {
                        className: "w-full bg-white"
                      }, React$1.createElement(MyInfo_Cash_Remain_Buyer.PC.make, {
                            query: query
                          })), React$1.createElement("div", {
                        className: "w-full mt-4 bg-white"
                      }, React$1.createElement(MyInfo_Processing_Order_Buyer.PC.make, {
                            query: query
                          })), React$1.createElement(MyInfo_Profile_Complete_Buyer.PC.make, {
                        query: query
                      }))
            });
}

var PC = {
  make: MyInfo_Buyer$PC
};

function MyInfo_Buyer$Mobile$Main(props) {
  var query = props.query;
  return React$1.createElement("section", undefined, React$1.createElement("div", {
                  className: "px-5 pt-5 pb-6"
                }, React$1.createElement("div", {
                      className: "mb-10"
                    }, React$1.createElement(MyInfo_ProfileSummary_Buyer.Mobile.make, {
                          query: query
                        })), React$1.createElement("div", {
                      className: "mb-12"
                    }, React$1.createElement(MyInfo_Cash_Remain_Buyer.Mobile.make, {
                          query: query
                        })), React$1.createElement("div", {
                      className: "mb-12"
                    }, React$1.createElement(MyInfo_Processing_Order_Buyer.Mobile.make, {
                          query: query
                        })), React$1.createElement(MyInfo_Profile_Complete_Buyer.Mobile.make, {
                      query: query
                    })), React$1.createElement("div", {
                  className: "h-3 bg-gray-100"
                }));
}

var Main = {
  make: MyInfo_Buyer$Mobile$Main
};

function MyInfo_Buyer$Mobile(props) {
  var to = new Date();
  var from = SubMonths(to, 1);
  var queryData = use({
        from: Format(from, "yyyyMMdd"),
        to: Format(to, "yyyyMMdd")
      }, undefined, undefined, undefined, undefined);
  var viewer = queryData.viewer;
  if (viewer !== undefined) {
    return React$1.createElement("div", {
                className: "w-full bg-white absolute top-0 pt-14 min-h-screen"
              }, React$1.createElement("div", {
                    className: "w-full max-w-3xl mx-auto bg-white h-full"
                  }, React$1.createElement("div", undefined, React$1.createElement("div", {
                            className: "flex flex-col"
                          }, React$1.createElement(MyInfo_Buyer$Mobile$Main, {
                                query: viewer.fragmentRefs
                              }), React$1.createElement("section", {
                                className: "px-4 mb-[60px]"
                              }, React$1.createElement("ol", undefined, React$1.createElement(Link, {
                                        href: "/buyer/me/account",
                                        children: React$1.createElement("li", {
                                              className: "py-5 flex justify-between items-center border-b border-gray-100"
                                            }, React$1.createElement("span", {
                                                  className: "font-bold"
                                                }, "계정정보"), React$1.createElement(IconArrow.make, {
                                                  height: "16",
                                                  width: "16",
                                                  fill: "#B2B2B2"
                                                }))
                                      }), React$1.createElement(Link, {
                                        href: "/buyer/upload",
                                        children: React$1.createElement("li", {
                                              className: "py-5 flex justify-between items-center border-b border-gray-100"
                                            }, React$1.createElement("span", {
                                                  className: "font-bold"
                                                }, "주문서 업로드"), React$1.createElement(IconArrow.make, {
                                                  height: "16",
                                                  width: "16",
                                                  fill: "#B2B2B2"
                                                }))
                                      }), React$1.createElement(Link, {
                                        href: "/buyer/orders",
                                        children: React$1.createElement("li", {
                                              className: "py-5 flex justify-between items-center border-b border-gray-100"
                                            }, React$1.createElement("span", {
                                                  className: "font-bold"
                                                }, "주문 내역"), React$1.createElement(IconArrow.make, {
                                                  height: "16",
                                                  width: "16",
                                                  fill: "#B2B2B2"
                                                }))
                                      }), React$1.createElement(Link, {
                                        href: "/buyer/transactions",
                                        children: React$1.createElement("li", {
                                              className: "py-5 flex justify-between items-center border-b border-gray-100"
                                            }, React$1.createElement("span", {
                                                  className: "font-bold"
                                                }, "결제 내역"), React$1.createElement(IconArrow.make, {
                                                  height: "16",
                                                  width: "16",
                                                  fill: "#B2B2B2"
                                                }))
                                      }), React$1.createElement(Link, {
                                        href: "/products/advanced-search",
                                        children: React$1.createElement("li", {
                                              className: "py-5 flex justify-between items-center border-b border-gray-100"
                                            }, React$1.createElement("span", {
                                                  className: "font-bold"
                                                }, "단품 확인"), React$1.createElement(IconArrow.make, {
                                                  height: "16",
                                                  width: "16",
                                                  fill: "#B2B2B2"
                                                }))
                                      }), React$1.createElement(Link, {
                                        href: "/buyer/download-center",
                                        children: React$1.createElement("li", {
                                              className: "py-5 flex justify-between items-center border-b border-gray-100"
                                            }, React$1.createElement("span", {
                                                  className: "font-bold"
                                                }, "다운로드 센터"), React$1.createElement(IconArrow.make, {
                                                  height: "16",
                                                  width: "16",
                                                  fill: "#B2B2B2"
                                                }))
                                      }), React$1.createElement(Link, {
                                        href: "https://drive.google.com/drive/u/0/folders/1DbaGUxpkYnJMrl4RPKRzpCqTfTUH7bYN",
                                        children: React$1.createElement("a", {
                                              rel: "noopener",
                                              target: "_blank"
                                            }, React$1.createElement("li", {
                                                  className: "py-5 flex justify-between items-center border-b border-gray-100"
                                                }, React$1.createElement("span", {
                                                      className: "font-bold"
                                                    }, "판매자료 다운로드"), React$1.createElement(IconArrow.make, {
                                                      height: "16",
                                                      width: "16",
                                                      fill: "#B2B2B2"
                                                    })))
                                      }), React$1.createElement(Link, {
                                        href: "https://shinsunmarket.co.kr/532",
                                        children: React$1.createElement("a", {
                                              target: "_blank"
                                            }, React$1.createElement("li", {
                                                  className: "py-5 flex justify-between items-center border-b border-gray-100"
                                                }, React$1.createElement("span", {
                                                      className: "font-bold"
                                                    }, "공지사항"), React$1.createElement(IconArrow.make, {
                                                      height: "16",
                                                      width: "16",
                                                      fill: "#B2B2B2"
                                                    })))
                                      })))))));
  } else {
    return null;
  }
}

var Mobile = {
  Main: Main,
  make: MyInfo_Buyer$Mobile
};

function $$default(param) {
  var router = Router.useRouter();
  return React$1.createElement("div", {
              className: "w-full min-h-screen"
            }, param.deviceType !== 1 ? React$1.createElement(React$1.Fragment, undefined, React.createElementWithKey(Header_Buyer.Mobile.make, {}, router.asPath), React$1.createElement(Authorization.Buyer.make, {
                        children: React$1.createElement(RescriptReactErrorBoundary.make, {
                              children: React$1.createElement(React$1.Suspense, {
                                    children: Caml_option.some(React$1.createElement(MyInfo_Buyer$Mobile, {})),
                                    fallback: Caml_option.some(React$1.createElement(MyInfo_Skeleton_Buyer.Mobile.Main.make, {}))
                                  }),
                              fallback: (function (param) {
                                  return React$1.createElement("div", undefined, "계정정보를 가져오는데 실패했습니다");
                                })
                            }),
                        title: "신선하이",
                        ssrFallback: Caml_option.some(React$1.createElement(MyInfo_Skeleton_Buyer.Mobile.Main.make, {}))
                      })) : React$1.createElement(React$1.Fragment, undefined, React$1.createElement("div", {
                        className: "flex"
                      }, React.createElementWithKey(Header_Buyer.PC.make, {
                            gnbBanners: param.gnbBanners,
                            displayCategories: param.displayCategories
                          }, router.asPath)), React$1.createElement(Authorization.Buyer.make, {
                        children: React$1.createElement(RescriptReactErrorBoundary.make, {
                              children: React$1.createElement(React$1.Suspense, {
                                    children: Caml_option.some(React$1.createElement(MyInfo_Buyer$PC, {})),
                                    fallback: Caml_option.some(React$1.createElement(MyInfo_Skeleton_Buyer.PC.make, {}))
                                  }),
                              fallback: (function (param) {
                                  return React$1.createElement("div", undefined, "계정정보를 가져오는데 실패했습니다");
                                })
                            }),
                        title: "신선하이",
                        ssrFallback: Caml_option.some(React$1.createElement(MyInfo_Skeleton_Buyer.PC.make, {}))
                      }), React$1.createElement(Footer_Buyer.PC.make, {})));
}

function getServerSideProps(ctx) {
  var deviceType = DeviceDetect.detectDeviceFromCtx2(ctx.req);
  var gnb = function (param) {
    return GnbBannerList_Buyer.Query.fetchPromised(RelayEnv.envSinsunMarket, undefined, undefined, undefined, undefined);
  };
  var displayCategories = function (param) {
    return ShopCategorySelect_Buyer.Query.fetchPromised(RelayEnv.envSinsunMarket, {
                onlyDisplayable: true,
                parentId: undefined,
                types: ["NORMAL"]
              }, undefined, undefined, undefined);
  };
  return Js_promise.$$catch((function (err) {
                console.log("에러 gnb preload", err);
                return Promise.resolve({
                            props: {
                              deviceType: deviceType,
                              gnbBanners: [],
                              displayCategories: []
                            }
                          });
              }), Js_promise.then_((function (param) {
                    return Promise.resolve({
                                props: {
                                  deviceType: deviceType,
                                  gnbBanners: param[0].gnbBanners,
                                  displayCategories: param[1].displayCategories
                                }
                              });
                  }), Promise.all([
                      gnb(undefined),
                      displayCategories(undefined)
                    ])));
}

export {
  Query ,
  PC ,
  Mobile ,
  $$default ,
  $$default as default,
  getServerSideProps ,
}
/* React Not a pure module */
