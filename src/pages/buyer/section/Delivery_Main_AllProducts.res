module Query = %relay(`
  query DeliveryMainAllProductsQuery(
    $count: Int
    $cursor: String
    $sort: ProductsQueryInputSort!
    $onlyBuyable: Boolean
  ) {
    ...DeliveryMainAllProductsFragment
      @arguments(
        count: $count
        cursor: $cursor
        sort: $sort
...
module Fragment = %relay(`
  fragment DeliveryMainAllProductsFragment on Query
  @refetchable(queryName: "DeliveryMainAllProductsRefetchQuery")
  @argumentDefinitions(
    ...
    ) @connection(key: "SectionDeliveryAll_products") {
      edges {
        cursor
        node {
          ...DeliveryProductListItemFragment
        }
      }
    }
  }
      ...
      <div className=%twc("w-[1280px] pt-20 pb-16 mx-auto min-h-full")>
        ...
        <ol className=%twc("grid grid-cols-4 gap-x-10 gap-y-16")>
          ...
          ->Array.map(({cursor, node}) => {
            <DeliveryProductListItem.PC key=cursor query=node.fragmentRefs />
          })
          ->React.array}
        </ol>
  ...
  module Container = {
    ...
    let make = () => {
      ...
      let sort =
        router.query
        ->Js.Dict.get("sort")
        ->Option.flatMap(DeliverySortSelect.decodeSort)
      ...
      | false =>
        <div className=%twc("w-full pt-1 px-5")>
          <div className=%twc("mb-4 w-full flex items-center justify-between text-gray-800")>
            <span className=%twc("font-bold")> {`전체 신선배송 상품`->React.string} </span>
            <DeliverySortSelect.MO />
          </div>
          <ol className=%twc("grid grid-cols-2 gap-x-4 gap-y-8")>
            {products.edges
            ->Array.map(({cursor, node}) => {
              <DeliveryProductListItem.MO key=cursor query=node.fragmentRefs />
            })
            ->React.array}
          </ol>
          <div ref={ReactDOM.Ref.domRef(loadMoreRef)} className=%twc("h-20 w-full") />
