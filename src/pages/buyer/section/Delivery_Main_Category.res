module Query = %relay(`
  query DeliveryMainCategoryQuery {
  ...
  module PC = {
    ...
    let make = (~id, ~src, ~name) => {
      ...
      <li className=%twc("cursor-pointer")>
        <Next.Link href={`/buyer/delivery/products?${queryStr}`}>
...
module PC = {
    ...
    let make = () => {
      <div
        ...
        <ol className=%twc("w-full flex items-center ")>
          ...
          ->Array.map(index => {
            let key = `delivery-main-category-placeholder-${index->Int.toString}`
  ...
  @react.component
  let make = () => {
    let query: DeliveryMainCategoryQuery_graphql.Types.response = Query.use(~variables=(), ())
