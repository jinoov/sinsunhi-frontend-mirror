...
let decodeSort = sortStr => {
  ...
  | "price-per-kg-asc" => Some(#PRICE_PER_KG_ASC)
  | _ => None
  }
}
    ...
    <Root _open onOpenChange={to_ => setOpen(._ => to_)}>
      ...
      <Content
        ...
        <Item
          ...
          className=%twc(
            "w-[140px] p-2 focus:outline-none text-gray-800 hover:bg-gray-100 rounded-lg cursor-pointer"
          )>
          {#UPDATED_DESC->makeSortLabel->React.string}
        </Item>
        <Item
          onSelect={#PRICE_PER_KG_ASC->makeOnSelect}
          className=%twc(
            "w-[140px] p-2 focus:outline-none text-gray-800 hover:bg-gray-100 rounded-lg"
          )>
          {#PRICE_PER_KG_ASC->makeSortLabel->React.string}
        </Item>
      </Content>
    </Root>
  </div>
