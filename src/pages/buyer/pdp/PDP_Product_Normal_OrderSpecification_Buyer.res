...
module Query = %relay(`
  query PDPProductNormalOrderSpecificationBuyerQuery($id: ID!) {
    node(id: $id) {
      ... on ProductOption {
        id
        optionName
...
let make = (~selectedSkuId, ~quantity, ~setQuantity) => {
  ...
  <Tabs.Root
    ...
    | Some({
      ...
      let deliveryCostLabel = `${(deliveryCost * quantity)
        ->Int.toFloat
            ...
            <section className=%twc("mt-2 flex flex-col")>
              ...
              <Next.Link
                ...
                <a
                  ...
                  <IconArrow width="20" height="20" fill=`#12B564` />
                </a>
              </Next.Link>
            </section>
          </div>
