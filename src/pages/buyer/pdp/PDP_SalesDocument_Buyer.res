...
module Fragment = %relay(`
  fragment PDPSalesDocumentBuyerFragment on Product {
    salesDocument
  }
`)
...
module MO = {
  ...
  let make = (~query) => {
    ...
    let {salesDocument} = Fragment.use(query)
...
module PC = {
  ...
  let make = (~query) => {
    ...
    | Some(downloadLink) => <>
        <div
          className=%twc(
            "mt-12 w-full p-5 rounded-xl bg-gray-50 flex items-center justify-between"
          )>
          <div className=%twc("flex flex-col text-gray-800")>
            <span>
              <span className=%twc("font-bold")> {`판매자료 다운받기`->React.string} </span>
