...
module Make = (
  ...
  let createResolverFromResult = (
    resolver: Form.t => Result.t<Form.t, Js.Dict.t<Error.t>>,
    data: Form.t,
  ) => {
  ...
  module MakeInput = (T: CreateFormInputConfig) => {
    ...
    @send external _setError: (t, string, Error.t, ~option: 'a=?, unit) => unit = "setError"
    let setError = (form, error) => form->_setError(T.name, error, ())
    let setErrorWithOption = (form, error, ~shouldFocus: bool) =>
      form->_setError(T.name, error, ~option={"shouldFocus": shouldFocus}, ())
  ...
  module MakeInputArray = (
    ...
    type fieldArrayState = {
      ...
      isTouched: bool,
      invalid: bool,
      error: option<array<option<Js.Dict.t<Error.t>>>>,
    }

    @send
    external _getFieldArrayState: (t, string, formState) => fieldArrayState = "getFieldState"
    let getFieldArrayState = form => form->_getFieldArrayState(T.name, form->formState)
    ...
    let registerWithIndex = (
      ...
      form->_register(
        `${T.name}.${index->Int.toString}${property != "" ? `.${property}` : ""}`,
